...
static const RCSwitch::Protocol PROGMEM proto[] = {
  ...
  { 350, {  1, 31 }, {  1,  3 }, {  3,  1 }, false }      // protocol 7 (Ying's Protocol)
};

static const int kDefaultProtocol = 7; // protocol numbers are 1-indexed
...
RCSwitch::RCSwitch() {
  ...
  this->setProtocol(kDefaultProtocol);
  #if not defined( RCSwitchDisableReceiving )
...
#else // Arduino
    int trueInterrupt = digitalPinToInterrupt(this->nReceiverInterrupt);
    attachInterrupt(trueInterrupt, handleInterrupt, CHANGE);
#endif
  }
}
...
bool RECEIVE_ATTR RCSwitch::receiveProtocol(const int p, unsigned int changeCount) {
    ...
    for (unsigned int i = firstDataTiming; i < changeCount - 1; i += 2) {
        ...
        if (diff(RCSwitch::timings[i], delay * pro.zero.high) < delayTolerance &&
            ...
            // Failed
//            Serial.println("failed here in some code shit");
            return false;
        }
    }
...
void RECEIVE_ATTR RCSwitch::handleInterrupt() {
  ...
  if (duration > RCSwitch::nSeparationLimit) {
    ...
    if (diff(duration, RCSwitch::timings[0]) < 200) {
      ...
      repeatCount++;
      if (repeatCount == 2) {
        boolean received = receiveProtocol(kDefaultProtocol, changeCount);
